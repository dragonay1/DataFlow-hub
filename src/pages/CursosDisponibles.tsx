    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cursos.map(curso => (
          <div
            key={curso.id}
            className={`bg-white p-6 rounded shadow ${cursoSeleccionado?.id === curso.id ? 'border-2 border-green-500' : ''}`}
          >
            <h2 className="text-lg font-semibold">{curso.nombre}</h2>
            <p>Profesor: {curso.profesor}</p>
            <p>Aula: {curso.aula}</p>

            <button onClick={() => setCursoSeleccionado(curso)} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
              Seleccionar Curso
            </button>
          </div>
        ))}
